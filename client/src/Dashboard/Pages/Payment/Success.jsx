...
import { Link, useSearchParams } from "react-router-dom"; // or Next.js router
import tick from '../../../../public/imgs/right.png'
...
const Success = () => {
  ...
  useEffect(() => {
    if (sessionId) {
      axios.get(`${import.meta.env.VITE_SERVER_URL}/api/subscription/payment-details?session_id=${sessionId}`)
  ...
  return (
   ...
   <img src={tick} className= " w-15 h-15" alt="" />
   <h1 className="xs:text-2xl text-5xl">Payment Successful</h1>
   </div>
      {sessionDetails ? (
